 ...
 *
 * API-сервер для управления заявками и книгами в системе AnalyseBooks.
 */
export class Api<
...
> extends HttpClient<SecurityDataType> {
  analyseBooks = {
     ...
     * @request POST:/analyse-books/draft/books/{book_id}
     * @secure
    ...
    draftBooksCreate: (bookId: number, params: RequestParams = {}) =>
      this.request<Record<string, string>, Record<string, string>>({
        path: `/analyse-books/draft/books/${bookId}`,
        method: "POST",
        secure: true,
        ...params,
      }),
